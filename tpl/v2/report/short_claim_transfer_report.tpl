...
<html>
<head>
<title>Claim Transfer Image Report</title>
...
<div id="pleasewait" style="position:absolute; visibility:hidden; z-index:2;"><img alt="" src="<TMPL_VAR NAME=IMGPATH>/arrows_wait_br_full.gif" id="pw" name="pw" width="400" height="120" border="0"></div>

<form name="SHORT_CLAIM_TRANSFER_REPORT" action="<TMPL_VAR NAME=ACTION_DOCKET>" method="post"> 
    <script>
    function update_claim_num_info(cost,clm_dtl_id, clm_num, court_loc, image_type, moxie_court_code, case_internal_id, dtl_num, CSRF_TOKEN) {
        ...
        var detail_num_div_id = document.getElementById("detail_num");
        detail_num_div_id.innerHTML = detail_num_div_id.innerHTML.replace(/\d+/,detail_num);
        ...
        qry_string = qry_string.replace(/internal-case-id=\d+$/,'internal_case_id='+case_internal_id);
        qry_string = qry_string.replace(/detail_num=\d+$/,'detail_num='+dtl_num);
        qry_string = qry_string.replace(/CSRF_TOKEN=\d+$/,'CSRF_TOKEN='+CSRF_TOKEN);

        document.forms[0].DETAIL_NUM.value = dtl_num;
        ...
        document.forms[0].INTERNAL_CASE_ID.value = case_internal_id;
        document.forms[0].CSRF_TOKEN.value = CSRF_TOKEN;
    ...
    <INPUT type=hidden NAME="INTERNAL_CASE_ID" VALUE="<TMPL_VAR NAME="INTERNAL_CASE_ID">">
    ...
    <INPUT type=hidden NAME="DETAIL_NUM" VALUE="<TMPL_VAR NAME="DETAIL_NUM">">
        ...
        <tr>
            <td align="center" colspan="2" >
                <table border="0" cellpadding="2" cellspacing="0" bgcolor="#000000">
                    <tr>
                        <td colspan="2" ><input name="BUTTON" tabindex="1" class="rb" type="button" value="Purchase Report" onMouseDown="flip_style(this,'rbd');"onClick="showpw();send_event('DOCKET/SHOW_CLAIM_TRANSFER_REPORT');disable_buttons('BUTTON');" onMouseOut="flip_style(this,'rb');"></td>        
                    </tr>
                </table>
            </td>
        </tr>
...
<table border="1" cellpadding="0" cellspacing="0" width="640" bgcolor="#ffffff">
  <tr>
	...
	<td valign="top" align="left" class="unifont1" nowrap>&nbsp;&nbsp;</td>
	<td valign="top" align="left" class="smallfont1" id="claim-num"><b><TMPL_VAR NAME=CLAIM_DETAIL_ID> - <TMPL_VAR NAME=CLAIM_NUM> -  <TMPL_VAR NAME=DETAIL_NUM></b></td>
      </tr>
    </table>
    </div>  
    </td>
					...
					<tr>
	  					<TMPL_IF NAME="SUB_CLAIM_NUM">
	    					...
	    					<td class="smallfont1" width="15%" valign="top">
								&nbsp;&nbsp;<input type="radio" name="SUB_CLAIM_NUM" onClick="document.forms[0].REPORT_TOTAL.value = '<TMPL_VAR NAME=ACCURINT_COST>';" value="<TMPL_VAR NAME="SUB_CLAIM_NUM">" value="<TMPL_VAR NAME="DETAIL_NUM">" <TMPL_IF NAME="DEFAULT">CHECKED</TMPL_IF>>
	    					</td>
						<TMPL_ELSE>
						   <td class="smallfont1" width="15%" valign="top">
								&nbsp;&nbsp;<input type="radio" name="CLAIM_DETAIL_ID" onClick="update_claim_num_info('<TMPL_VAR NAME=ACCURINT_COST>', '<TMPL_VAR NAME=CLAIM_DETAIL_ID>', '<TMPL_VAR NAME=CLAIM_NUM>', '<TMPL_VAR NAME=COURT_LOC>', '<TMPL_VAR NAME=IMAGE_TYPE>', '<TMPL_VAR NAME=MOXIE_COURT_CODE>', '<TMPL_VAR NAME=INTERNAL_CASE_ID>', '<TMPL_VAR NAME=DETAIL_NUM>', '<TMPL_VAR NAME=CSRF_TOKEN>');" value="__NOT_DEFINED__" <TMPL_IF NAME="DEFAULT">CHECKED</TMPL_IF>>
	    					</td>
						</TMPL_IF>
