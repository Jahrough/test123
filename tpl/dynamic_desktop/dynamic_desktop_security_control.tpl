...
<script type="text/javascript">
    function _chat_disclaimer(options) {
        ...
        if (chat_disabled || 1 != options) {
            var chat_disclaimer_html = '<strong>By enabling the Chat feature, you are choosing to allow certain information that users have submitted to be shared with other parties.</strong>';
            if (options) {
            ...
            build_tip('Chat Disclaimer',chat_disclaimer_html,'',1,options);
        }
    }

    function _set_chat_accepted(flag) {
        document.forms[0].ENABLE_CHAT_ACCESS[ flag ? 0 : 1 ].checked = true;
        close_tip('info_rem','wrapperoverlay');
    }
</script>
<TMPL_IF ENABLE_V2>
