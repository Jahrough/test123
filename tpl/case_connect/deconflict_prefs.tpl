...
<TMPL_IF NAME="PREFERENCES_ERROR_MESSAGE">
  ...
  <div class="smallfont1" style="border: #336699 solid; border-width: 2px; padding: 2px;  text-align: left;">
    <div style="text-align: left; padding: 4px;">
      <span class="myaccountlabel">Case Deconfliction Alert (Enrollment):</span>
      <span style="padding: 2px 25px 2px 10px;">
        <input id="enroll_en" type="radio" name="enroll" value="yes"<TMPL_IF
       DISABLE_CASE_CONNECT_ENROLL> disabled="true"<TMPL_ELSE><TMPL_IF
       NAME=enroll> checked="checked"</TMPL_IF></TMPL_IF>
         onclick="javascript:check_case_connect();"/>
        <label for="enroll_en">Enabled for all searches</label>
      </span>
      <span style="padding: 2px 25px 2px 10px;">
        <input id="enroll_dis" type="radio" name="enroll" value="no"<TMPL_IF
       DISABLE_CASE_CONNECT_ENROLL> disabled="true" checked="checked"<TMPL_ELSE><TMPL_UNLESS enroll> checked="checked"</TMPL_UNLESS>
         onclick="javascript:check_case_connect();"/></TMPL_IF>
        <label for="enroll_dis">Disabled for all searches</label>
      </span>
    </div>
    <div style="text-align: left; padding: 4px;">
      <span class="myaccountlabel">Case Deconfliction Opt-In:</span>
      <span style="padding: 2px 25px 2px 10px;">
        <input id="opt_in_en" type="radio" name="opt_in" value="yes"<TMPL_IF
       DISABLE_CASE_CONNECT_OPT_IN> disabled="true"<TMPL_ELSE><TMPL_IF opt_in>checked="checked" </TMPL_IF></TMPL_IF>
         onclick="javascript:check_case_connect();"/>
        <label for="opt_in_en">Enabled for all searches</label>
      </span>
      <span style="padding: 2px 25px 2px 10px;">
        <input id="opt_in_dis" type="radio" name="opt_in" value="no"<TMPL_IF
       DISABLE_CASE_CONNECT_OPT_IN> disabled="true" checked="checked"<TMPL_ELSE><TMPL_UNLESS
       NAME=opt_in> checked="checked"</TMPL_UNLESS></TMPL_IF>
         onclick="javascript:check_case_connect();"/>
        <label for="opt_in_dis">Disabled for all searches</label>
      </span>
    </div>
    <div style="text-align: left; padding: 10px 4px 10px 4px;">
      Note: You will have to override this enabled/disabled setting per search if you
    ...
    <div style="text-align: left; padding: 4px;">
      <span style="display: inline-block; padding: 2px 35px 20px 10px;">
        <input id="self_cb" type="checkbox" name="self" value="yes"<TMPL_IF
         NAME=CC_DISABLED1> disabled="true"<TMPL_ELSE><TMPL_IF
         NAME=CC_CHECK1> checked="checked"</TMPL_IF></TMPL_IF>
         onclick="javascript:check_case_connect();"/>
        <label for="self_cb">Internal to My Account</label>
        <span>
          <a href="javascript:void(0);" style="cursor: help;"
           onClick="javascript:open_help_html('case_deconfliction_help.html#options','Deconfliction');">
            <img src="<TMPL_VAR NAME=IMGPATH>/v2/ico_question.gif" alt="More Information"
             border="0" height="13" hspace="5" width="14"/>
          </a>
        </span>
        <br/>
        <input id="company_cb" type="checkbox" name="company" value="yes"<TMPL_IF
         NAME=CC_DISABLED2> disabled="true"<TMPL_ELSE><TMPL_IF
         NAME=CC_CHECK2> checked="checked"</TMPL_IF></TMPL_IF>
         onclick="javascript:check_case_connect();"/>
        <label for="company_cb">Internal to My Parent Company (Agency)</label>
      </span>
      <span style="display: inline-block; padding: 2px 35px 20px 10px;">
        <input id="le_users_cb" type="checkbox" name="le_users" value="yes"<TMPL_IF
         NAME=CC_DISABLED3> disabled="true"<TMPL_ELSE><TMPL_IF
         NAME=CC_CHECK3> checked="checked"</TMPL_IF></TMPL_IF>
         onclick="javascript:check_case_connect();"/>
        <label for="le_users_cb">External (Other Law Enforcement Users)</label>
        <br/>
        <input id="gov_users_cb" type="checkbox" name="gov_users" value="yes"<TMPL_IF
         NAME=CC_DISABLED4> disabled="true"<TMPL_ELSE><TMPL_IF
         NAME=CC_CHECK4> checked="checked"</TMPL_IF></TMPL_IF>
         onclick="javascript:check_case_connect();"/>
