<!-- begin tpl/popup/legends/legend_business.tpl -->

<TMPL_INCLUDE NAME="legend_header.tpl">

<table border="0" cellpadding="0" cellspacing="2" width="308">
  <tr>
    <td colspan="2" align="center" class="style2"><b>Icon Legend</b></td>
  </tr>
  <tr>
    <td colspan="2"><img src="<TMPL_VAR NAME=IMGPATH>/spacer.gif" width="1" height="10" border="0"></td>
  </tr>
  <tr>
    <td width="25" align="center"><img src="<TMPL_VAR NAME=IMGPATH>/bus_big2.gif" width="19" height="24" border="0"></td>
    <td width="289" class="style1"><B>Comprehensive Report</B></td>
  </tr>
  <tr>
    <td class="style1">&nbsp;</td>
    <td class="style1">This icon can be found on the far left of the search results.&nbsp; If you wish to create a <b>Comprehensive Report</b> for any record, click this icon.</td>
  </tr>
  <tr>
    <td colspan="2"><img src="<TMPL_VAR NAME=IMGPATH>/spacer.gif" width="1" height="10" border="0"></td>
  </tr>
  <script>
    custom_legend();
  </script>
  <tr>
    <td width="25" align="center"><img src="<TMPL_VAR NAME=IMGPATH>/address_big2.gif" width="21" height="21" border="0"></td>
    <td width="289" class="style1"><B>Address Report</B></td>
  </tr>
  <tr>
    <td class="style1">&nbsp;</td>
    <td class="style1">This icon can be found on the right of the search results.&nbsp; If you wish to create an <b>Address Report</b> for any record, click this icon.</td>
  </tr>
  <tr>
    <td colspan="2"><img src="<TMPL_VAR NAME=IMGPATH>/spacer.gif" width="1" height="10" border="0"></td>
  </tr>
  <tr>
    <td width="25" align="center"><img src="<TMPL_VAR NAME=IMGPATH>/bluecheck.gif" width="16" height="16" border="0"></td>
    <td width="289" class="style1"><B>Probable Active Phone</B></td>
  </tr>
  <tr>
    <td class="style1">&nbsp;</td>
    <td class="style1">This icon is found to the immediate left of the <b>Phone number column</b> when the data indicates that this is a probable active phone.</td>
  </tr>
  <tr>
    <td colspan="2"><img src="<TMPL_VAR NAME=IMGPATH>/spacer.gif" width="1" height="10" border="0"></td>
  </tr>
</table>

<TMPL_INCLUDE NAME="legend_footer.tpl">

<!-- end tpl/popup/legends/legend_business.tpl -->
