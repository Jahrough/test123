<html>
<head>
<TMPL_IF NAT_ACC_ENABLE>
	<title>National Motor Vehicle Accidents</title>
<TMPL_ELSE>
	<title>Florida Accidents</title>
</TMPL_IF>
<link href="<TMPL_VAR NAME='CSSPATH'>/<TMPL_VAR NAME=SKIN_TYPE>/popups.css" rel="stylesheet" type="text/css">
<TMPL_IF ENABLE_V2>
<style type="text/css">
body {
        color:#333333;
        background:#FCFAF5;
        font-family:Verdana, Arial, Helvetica, sans-serif;
        line-height:110%;
}
</style>
</TMPL_IF>
</head>

<body onBlur="window.close();" topmargin="0" leftmargin="0" marginwidth="0" marginheight="0">

<div align="center">

<br>
<table border="0" cellpadding="0" cellspacing="0" width="308">
  <tr>
    <td align="center">
    <br>
    <table border="0" cellpadding="0" cellspacing="0">
      <tr>
        <td align="center" class="popuptitle">
			<B>
				<TMPL_IF NAT_ACC_ENABLE>
					National Motor Vehicle Accidents
				<TMPL_ELSE>
					Florida Accidents
				</TMPL_IF>					
			</B>
        </td>
      </tr>
    </table>
    </td>
  </tr>
</table>
<table border="0" cellpadding="20" cellspacing="0" width="308">
  <tr>
    <td class="popupbody" width="308" align="left">
    Using our advanced technology and searching logic we have also queried
    many of our other data sources for information about your subject which,
    combined with our proprietary algorithms, allow us to return these
    potential results for your subject.
    </td>
  </tr>
</table>
<br>
<!--
<a href="javascript:parent.close();"><img src="<TMPL_VAR NAME=IMGPATH>/close.gif" width="54" height="29" border="0"></a>
-->
</div>
</body>
</html>